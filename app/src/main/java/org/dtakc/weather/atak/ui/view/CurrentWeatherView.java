...
public class CurrentWeatherView {
    ...
    public void bindCurrentWeather(WeatherModel weather, String requestTime) {
        ...
        if (textRealFeel != null)
            textRealFeel.setText(String.format("%.1f°C", weather.apparentTemperature));
        if (textVisibility != null)
            textVisibility.setText(String.format("%.0f m", weather.visibility));
        if (textHumidity != null)
            textHumidity.setText(String.format("%.0f %%", weather.humidity));
        if (textPressure != null)
            textPressure.setText(String.format("%.1f hPa", weather.pressure));
        if (textWind != null)
            textWind.setText(String.format("%.1f m/s / %.0f°",
                    weather.windSpeed, weather.windDirection));

        bindPrecipitation(weather.precipitationSum, weather.precipitationHours);
        applyWmoCode(weather.weatherCode);
    ...
    public void bindMetar(WeatherModel weather) {
        ...
        if (textMetarStation != null)
            textMetarStation.setText(weather.icaoId.isEmpty() ? "—" : weather.icaoId);

        if (textMetarFltCat != null) {
            String cat = weather.flightCategory;
