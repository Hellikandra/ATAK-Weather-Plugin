...
public class ComparisonView {
    ...
    private void bindCard(WeatherModel w,
        ...
        if (coords != null && w.latitude != 0)
            coords.setText(String.format("%.4f, %.4f", w.latitude, w.longitude));

        WmoCodeMapper.WmoInfo info = WmoCodeMapper.resolve(w.weatherCode);
        if (icon != null) icon.setImageResource(info.drawableResId);
        if (desc != null) desc.setText(info.labelResId);

        setText(temp,     String.format("%.1f / %.1f°C", w.temperatureMin, w.temperatureMax));
        setText(feels,    String.format("%.1f°C",  w.apparentTemperature));
        setText(humidity, String.format("%.0f %%", w.humidity));
        setText(pressure, String.format("%.1f hPa",w.pressure));
        setText(wind,     String.format("%.1f m/s  %.0f°", w.windSpeed, w.windDirection));
        setText(precip,   w.precipitationSum == 0
                ? "—"
                : String.format("%.1f mm", w.precipitationSum));
    }
