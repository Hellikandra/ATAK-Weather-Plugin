 ...
 * Up to {@link #MAX_SLOTS} independent wind profile fetches can coexist.
 *
 * <h3>Refactoring changes</h3>
 * <ul>
 *   <li>{@link WindSlot} mutable fields made package-private with read-only accessors.</li>
 *   <li>Typed update methods: {@link #updateSlotRange}, {@link #updateSlotHeight},
 *       {@link #updateSlotSource}.</li>
 *   <li>{@link #loadWindProfile} marked {@code @Deprecated}.</li>
 *   <li><b>New:</b> {@link #selectedHourIndex} — shared between the DDR SeekBar
 *       ({@code WindTabCoordinator}) and the map HUD ({@code WindHudWidget}).
 *       Both call {@link #setHourIndex(int)}; both observe {@link #getSelectedHour()}.</li>
 * </ul>
 */
public class WindProfileViewModel extends ViewModel {
    ...
    public static class WindSlot {
        ...
        String sourceId;
        double rangeM  = 2_000.0;
        double heightM =   500.0;
    ...
    /**
     * Shared hour-scrubber index (0 = current hour, up to 167 h ahead).
     * Observed by both {@code WindTabCoordinator} in the DDR and
     * {@code WindHudWidget} on the map. Both call {@link #setHourIndex(int)}.
     */
    private final MutableLiveData<Integer> selectedHourIndex = new MutableLiveData<>(0);
    ...
    // ── Hour scrubber (shared DDR ↔ HUD) ─────────────────────────────────────

    public LiveData<Integer> getSelectedHour() { return selectedHourIndex; }

    /** Call from DDR SeekBar or HUD SliderWidget — both UIs react to the emission. */
    public void setHourIndex(int hourIndex) {
        selectedHourIndex.setValue(Math.max(0, hourIndex));
    }

    public int getCurrentHourIndex() {
        Integer v = selectedHourIndex.getValue();
        return v != null ? v : 0;
    }

    // ── Legacy single-slot API ────────────────────────────────────────────────

    /** @deprecated Use {@link #addSlot(double, double, String)} instead. */
    @Deprecated
    public void loadWindProfile(double lat, double lon) {
        ...
        weatherRepository.getWindProfile(lat, lon,
                new IWeatherRepository.Callback<List<WindProfileModel>>() {
                    @Override public void onSuccess(List<WindProfileModel> r) { windProfile.setValue(UiState.success(r)); }
                    @Override public void onError(String m)                    { windProfile.setValue(UiState.error(m)); }
                });
    }
    ...
    public int addSlot(double lat, double lon) { return addSlot(lat, lon, ""); }
    public int addSlot(double lat, double lon, String sourceId) { return addSlot(lat, lon, sourceId, null); }
    ...
    public void refetchSlot(int listIndex, String newSourceId) { updateSlotSource(listIndex, newSourceId); }

    // ── Private helpers ───────────────────────────────────────────────────────

    private WindSlot slotAt(int idx) {
        List<WindSlot> slots = getSlotList();
        return (idx >= 0 && idx < slots.size()) ? slots.get(idx) : null;
    }
