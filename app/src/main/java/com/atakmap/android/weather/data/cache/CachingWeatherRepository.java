...
public class CachingWeatherRepository implements IWeatherRepository {
    ...
    // ── Wind profile in-memory cache (ISS-04) ────────────────────────────────
    //
    // The wind profile endpoint is called every time the hour seekbar moves.
    // Without caching this floods the network with identical requests.
    // We keep the last successful result keyed by rounded (lat, lon) and evict
    // it after WIND_TTL_MS (30 minutes — wind aloft changes slowly).
    //
    // This is intentionally a simple in-memory map rather than a Room table:
    // wind profiles are large (5 altitudes × 168 hours), change faster than
    // surface weather, and do not need to survive an app restart.
    ...
    private static class WindCacheEntry {
        ...
        WindCacheEntry(List<WindProfileModel> data, long fetchedAt) {
            this.data = data; this.fetchedAt = fetchedAt;
        }
        boolean isFresh() {
    ...
    /** Key: "lat_lon" rounded to 2 decimal places (~1 km). */
    private final java.util.Map<String, WindCacheEntry> windCache =
     ...
     *
     * Cache is checked synchronously on the calling thread (main thread via
     * WindProfileViewModel), so cache hits are instant — no executor dispatch.
     * On a miss, the network call proceeds as before and populates the cache
     * on success.
     */
    @Override
    public void getWindProfile(double lat, double lon,
        ...
        network.getWindProfile(lat, lon, new Callback<List<WindProfileModel>>() {
            ...
            @Override public void onError(String message) {
                // Serve stale cached data if available rather than surfacing an error
