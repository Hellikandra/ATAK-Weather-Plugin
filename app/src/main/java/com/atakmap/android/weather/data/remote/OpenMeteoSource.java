...
public class OpenMeteoSource implements IWeatherRemoteSource,
    ...
    // NOTE: Open-Meteo API v1 renamed fields (2024): windspeed→wind_speed, etc.
            ...
            public void onSuccess(String body) {
                try {
                    ...
                    WeatherModel model = new WeatherModel.Builder(
                            ...
                            .humidity(hourly.getJSONArray("relative_humidity_2m").getDouble(0))
                            .pressure(hourly.getJSONArray("surface_pressure").getDouble(0))
                            .visibility(hourly.getJSONArray("visibility").getDouble(0))
                            .windSpeed(hourly.getJSONArray("wind_speed_10m").getDouble(0))
                            .windDirection(hourly.getJSONArray("wind_direction_10m").getDouble(0))
        ...
        HttpClient.get(url, new HttpClient.Callback() {
            ...
            public void onSuccess(String body) {
                try {
                    ...
                    JSONArray humid   = hourly.getJSONArray("relative_humidity_2m");
                    JSONArray press   = hourly.getJSONArray("surface_pressure");
                    JSONArray vis     = hourly.getJSONArray("visibility");
                    JSONArray wspd    = hourly.getJSONArray("wind_speed_10m");
                    JSONArray wdir    = hourly.getJSONArray("wind_direction_10m");
                    JSONArray precPct = hourly.getJSONArray("precipitation_probability");

                    // Optional field — present only if user selected it
                    JSONArray wcode   = hourly.optJSONArray("weather_code");
        ...
        HttpClient.get(url, new HttpClient.Callback() {
            ...
            public void onSuccess(String body) {
                try {
                    ...
                    String[] speedKeys = {"wind_speed_10m","wind_speed_80m","wind_speed_120m","wind_speed_180m"};
                    String[] dirKeys   = {"wind_direction_10m","wind_direction_80m","wind_direction_120m","wind_direction_180m"};
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        for (int a = 0; a < altitudes.length; a++) {
                            double gusts = a == 0
                                    ? hourly.getJSONArray("wind_gusts_10m").getDouble(i) : 0;
